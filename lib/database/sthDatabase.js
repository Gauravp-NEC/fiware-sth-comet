...
function connect(params, callback) {
  connectionURL = 'mongodb://' + (params.authentication ? params.authentication + '@' : '') + params.dbURI + '/' + params.database +
    (params.replicaSet ? '?replicaSet=' + params.replicaSet : '') + (params.authSource ? (params.replicaSet ? '&authSource=' +
...
function fetchCollection(databaseName, isAggregated, shouldTruncate, shouldCreate, collectionName, callback) {	
  ...
  var connection = client.db(databaseName);
